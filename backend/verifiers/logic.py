...
class LogicVerifier:
    ...
    def verify(self, candidate, task, work_dir=None):
        try:
            ...
            if len(cs) > 12 or len(claim) > 200:
                raise ValueError("Constraint limits exceeded.")
            ...
            def expr(s):
                if (
                    ...
                    or len(s) > 200
                    or not re.fullmatch(r"[0-9xy+\-*/<>=() \t]+", s)
                ):
                    raise ValueError("Unsupported constraint syntax.")

                # Python's AST requires equality comparisons to use "==".
                # The demo accepts the more natural constraint notation "x = 5".
                # Normalize standalone "=" without changing >=, <=, ==, or !=.
                s = re.sub(r"(?<![<>=!])=(?!=)", "==", s)
            ...
            parts = [
                p.strip()
                for p in re.split(r"\band\b|,", claim, flags=re.I)
                if p.strip()
            ]
            ...
            if r == unsat:
                return VerificationResult(
                    ...
                    "Claim cannot satisfy all constraints.",
                    {"solver": "unsat"},
                )
